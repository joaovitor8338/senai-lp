...
SELECT c.nome AS categoria, COUNT(*) AS qt_filmes_G
FROM categoria AS c
JOIN filme_categoria AS fc ON c.categoria_id = fc.categoria_id
JOIN filme AS f ON f.filme_id = fc.filme_id
WHERE f.classificacao = 'G'
GROUP BY c.nome;

/* select count(*) quantidade, f.classificacao, c.categoria_id from filme f inner join filme_categoria
...
select c.nome, count(*) quantidade, f.classificacao from filme f inner join filme_categoria fc
on f.filme_id = fc.filme_id
inner join categoria c
